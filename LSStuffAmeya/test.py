...
class ADS1256:
    ...
    #Method cycles through Individual Channels for single ended measuremtns
        
    def ADS1256_GetAll(self):
        ADC_Value = [0,0,0,0,0,0,0,0]
        for i in range(0,8,1):
            ADC_Value[i] = self.ADS1256_GetChannalValue(i)*5.0/0x7fffff
        return ADC_Value


filepath = os.path.join('/home/pi/myPiFiles/LSStuffAmeya',time.strftime("%d-%m-%yat%I-%M%p"))
...
try:
    ADC = ADS1256()
    ADC.ADS1256_init() #Sets Data rate
    # ADC.ADS1256_SetMode(1) # Differential Measurements Actually set above
    firstt=int(datetime.utcnow().timestamp()*1e6)
    
    init_file()
    f.write("Gain: 1, DRate: 15000")
